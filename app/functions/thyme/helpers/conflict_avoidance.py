...
def get_suggested_start_time(ideal_start, ideal_end, awake_range, busy_ranges):
    ...
    while (delta < MAX_DELTA):
        ...
        for i in range(len(starts)):
            ...
            at_least_one_busy_range_in_target_range = False
            for busy_range in busy_ranges:
                busy_range_in_target_range = start_or_end_in_ranges(busy_range['start'], busy_range['end'], [{'start': start, 'end': end}])
                if busy_range_in_target_range:
                    at_least_one_busy_range_in_target_range = True
                    break
